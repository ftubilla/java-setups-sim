...
public abstract class ClearTheLargestWeightedDeviationPolicy extends ClearingPolicy {
	...
	/**
	 * Returns the weight of the deviation for choosing the
	 * item with the largest weighted deviation. Returns <code>null</code>
	 * if there is no weight for the given item.
	 * 
	 * @param item
	 * @return Double
	 */
	...
	/**
	 * Returns the item with largest weighted deviation (breaking ties by item id and
	 * excluding in such a case the current setup).
	 */
	@Override
	protected Item nextItem() {
		...
		for (Item item : machine){						
			...
			if (weight == null){
				log.fatal("No weight given for " + item + ". Check implementation of your policy"); //This will throw a NPE
			}
			double weightedDeviation = item.getSurplusDeviation() * weight;
			if (weightedDeviation > largestWeightedDeviation){
		...
		if (nextItem != null){
			...
			//If all items have the same weighted deviation, find the next item that's not the current setup		
			for (Item item : machine){
