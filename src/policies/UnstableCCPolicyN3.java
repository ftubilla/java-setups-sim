...
public class UnstableCCPolicyN3 implements IPolicy {
    ...
    public void updateControl(Sim sim) {
        // Strictly speaking, this example policy is intended for reliable
        // systems
            ...
            if (sim.getMachine().getOperationalState() != OperationalState.SETUP) {
                if (sim.getMachine().getSetup().onOrAboveTarget()) {
                    ...
                    if (changeTo == -1) {
                        ...
                        if (setup == 0) {
                            // If finished 0, go to the one with the *smallest*
                            // deviation!
                            if (y1 > y2) {
                                changeTo = 2;
                            } else {
                    ...
                    sim.getMasterScheduler()
                            .addEvent(new Changeover(sim.getTime(), sim.getMachine().getItemById(changeTo)));
