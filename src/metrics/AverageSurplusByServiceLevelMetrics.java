...
public class AverageSurplusByServiceLevelMetrics {
	...
	/**
	 * Finds the optimal offset of the trajectory for achieving the desired level of service.
	 * 
	 * @param item
	 * @param desiredServiceLevel
	 * @return Pair<Double, SurplusStatistics> A pair of the offset and surplus statistics values found.
	 */
	public Pair<Double, SurplusStatistics> findOptimalOffsetForServiceLevel(Item item, double desiredServiceLevel) {
		...
		SurplusStatistics lowerBoundStats = BatchSurplusStatisticsCalculator.translateAndCalculate(-originalTarget, dataPoints);
		SurplusStatistics upperBoundStats = BatchSurplusStatisticsCalculator.translateAndCalculate(-minSurplus, dataPoints);
		...
		//Convert the best target into an offset and return together with the new stats
		return Pair.of(target - originalTarget, newStats);
	}	
}
