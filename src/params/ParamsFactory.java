...
/**
 * Generates a series of <code>Params</code> instances based on a json file or
 * directory containing a series of json files.
 * 
 * @author ftubilla
 *
 */
...
public class ParamsFactory {
        ...
        if (jsonDir.isDirectory()) {
            for (final File jsonFile : jsonDir.listFiles()) {
                if (isJson(jsonFile)) {
                    ...
                    this.paramsList.add(readFile(jsonFile));
                } else {
    ...
    private boolean isJson(File file) {
        ...
        // Check that the last termination is json (there could be more than one
        // . in the file name)
        return splitByTermination[len - 1].equals("json");
    }

    private static Params readFile(File jsonFile) {
